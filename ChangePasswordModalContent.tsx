...
interface ChangePasswordModalContentProps {
  onChangePassword: (oldPassword: string, newPassword: string) => Promise<boolean>; // Returns Promise<true> on success
  onClose: () => void;
}
...
const ChangePasswordModalContent: React.FC<ChangePasswordModalContentProps> = ({ onChangePassword, onClose }) => {
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true);
    // onChangePassword is now async and returns a promise
    const success = await onChangePassword(currentPassword, newPassword);
    setIsLoading(false);
    
    // Success/error handling is now primarily managed by App.tsx via popup messages.
    // The `success` boolean can be used here if specific UI changes are needed in the modal upon success/failure,
    // beyond what App.tsx does. For now, App.tsx handles popups and closing the modal on success.
    if (!success && !error) { 
       // If onChangePassword itself doesn't set a specific error (e.g. via App.tsx popup),
       // a generic one could be set here, but usually App.tsx's feedback is sufficient.
    }
  };
