...
// Placeholder icons for profile fields
...
const TransactionIcon: React.FC<{ className?: string }> = ({ className="w-4 h-4" }) => <svg xmlns="http://www.w3.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>;
...
const AccordionSection: React.FC<AccordionSectionProps> = ({ title, badgeNumber, children, defaultOpen = false, isEditing, onEditToggle }) => {
  ...
  return (
    <div className="mb-3 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
      ...
      {isOpen && <div id={`accordion-content-${title.replace(/\s+/g, '-')}`} className={`p-4 border-t border-gray-200 dark:border-gray-700 ${isEditing ? 'pt-2' : ''}`}>{children}</div>}
    </div>
  );
};
...
const ProfileField: React.FC<{icon: React.ElementType, label: string, value?: string | null}> = ({icon: Icon, label, value}) => (
    <div className="flex items-start py-2.5">
        ...
        <div>
            ...
            <p className="text-sm text-gray-800 dark:text-gray-100 font-medium">{value || 'N/A'}</p>
        </div>
    </div>
);
...
const ProfileScreen: React.FC<ProfileScreenProps> = ({ user, onLogout, onUpdateProfile }) => {
  ...
  const tabs = ['INFO', 'ORDERS', 'COURSES', 'PERFORMANCE', 'PAYMENT']; 
  ...
  const handleEditToggle = (section: 'basic' | 'personal' | 'address') => {
    ...
    if (currentlyEditing) { 
      ...
      // Initialize with current user data, ensuring all possibly undefined fields are handled.
  ...
  const handleSave = (section: 'basic' | 'personal' | 'address') => {
    onUpdateProfile(editedUser); // App.tsx will handle Firestore update
    ...
    if (file && user?.email) { 
      ...
      reader.onloadend = async () => {
        ...
        try {
          ...
          const avatarStorageRef = storage.ref(`user_avatars/${userIdForPath}/${file.name}`); // Use compat storage.ref()
          
          await avatarStorageRef.putString(base64DataUrl, 'data_url'); // Use compat putString()
          const downloadURL = await avatarStorageRef.getDownloadURL(); // Use compat getDownloadURL()
  ...
  return (
    <div className="bg-gray-100 dark:bg-gray-900 min-h-full flex flex-col pb-16">
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex sticky top-0 z-10">
        {tabs.map((tab) => (
          <button
            ...
            className={`flex-1 py-3 text-center text-sm font-medium transition-colors focus:outline-none
              ${activeTab === tab 
      ...
      <div className="flex-grow p-4 overflow-y-auto">
          ...
          <div>
            <AccordionSection title="Basic Information" badgeNumber={1} defaultOpen={true} isEditing={isBasicInfoEditing} onEditToggle={() => handleEditToggle('basic')}>
              {isBasicInfoEditing ? (
                ...
                <>
                  ...
                  <ProfileField icon={CalendarFieldIcon} label="Date of Joining" value={user.dateOfJoining || "Jan 15, 2023"} />
                </>
              )}
            </AccordionSection>
            ...
            <AccordionSection title="Personal Details" badgeNumber={2} isEditing={isPersonalInfoEditing} onEditToggle={() => handleEditToggle('personal')}>
              {isPersonalInfoEditing ? (
                ...
                <>
                  <ProfileField icon={CalendarFieldIcon} label="Date of Birth" value={user.dateOfBirth} />
             ...
             <AccordionSection title="Address" badgeNumber={3} isEditing={isAddressEditing} onEditToggle={() => handleEditToggle('address')}>
                {isAddressEditing ? (
                  <>
                    <EditableProfileField label="Street Address" name="streetAddress" value={editedUser.streetAddress || user.streetAddress ||''} onChange={handleInputChange} placeholder="e.g., 123 Main St, Apt 4B" />
                    <EditableProfileField label="City" name="city" value={editedUser.city || user.city || ''} onChange={handleInputChange} />
                    <EditableProfileField label="State" name="state" value={editedUser.state || user.state || ''} onChange={handleInputChange} />
                    <EditableProfileField label="Postal Code" name="postalCode" value={editedUser.postalCode || user.postalCode || ''} onChange={handleInputChange} />
                    <EditableProfileField label="Country" name="country" value={editedUser.country || user.country || ''} onChange={handleInputChange} />
                  ...
                  <>
                    <ProfileField icon={AddressFieldIcon} label="Street Address" value={user.streetAddress} />
                    <ProfileField icon={AddressFieldIcon} label="City" value={user.city} />
                    <ProfileField icon={AddressFieldIcon} label="State" value={user.state} />
                    <ProfileField icon={HashFieldIcon} label="Postal Code" value={user.postalCode} />
                    <ProfileField icon={AddressFieldIcon} label="Country" value={user.country} />
                    {!user.streetAddress && !user.city && <p className="text-gray-500 dark:text-gray-400 text-sm p-2">No address details provided.</p>}
                  </>
                )}
            </AccordionSection>
                ...
                <div key={order.id} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">Date: { (order.orderDate instanceof Date) ? order.orderDate.toLocaleDateString() : new Date((order.orderDate as any)?.seconds * 1000).toLocaleDateString() }</p>
                    </div>
                  ...
                  <div className="mb-2">
                    {order.items.slice(0,2).map(item => ( 
                         <div key={item.id} className="flex items-center text-sm text-gray-600 dark:text-gray-300 py-0.5">
                            <img src={item.imageUrl} alt={item.name} className="w-6 h-6 rounded object-cover mr-2"/>
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-gray-100 dark:border-gray-700">
                     <p className="text-sm text-gray-600 dark:text-gray-300">Payment: <span className="font-medium">{order.paymentMethod}</span></p>
        ...
        {activeTab === 'COURSES' && <p className="text-gray-600 dark:text-gray-400 text-center p-5">Courses content goes here.</p>}
        {activeTab === 'PERFORMANCE' && <p className="text-gray-600 dark:text-gray-400 text-center p-5">Performance content goes here.</p>}
        {activeTab === 'PAYMENT' && <p className="text-gray-600 dark:text-gray-400 text-center p-5">Payment content goes here.</p>}
      </div>
    </div>
  );
};
