...
import TopBar from '../components/TopBar'; // Import TopBar for the header

// Placeholder icons for settings
...
const SettingsScreen: React.FC<SettingsScreenProps> = ({ version, onLogout, onOpenModal, currentTheme, onSetTheme, onBack, addPopupMessage }) => {
  ...
  const SettingRowToggle: React.FC<{icon: React.ReactNode, label: string, isChecked: boolean, onChange: () => void, id: string}> = 
    ...
    <div className="w-full flex justify-between items-center bg-white dark:bg-gray-800 p-3.5 rounded-lg shadow-sm"> {/* Reduced padding */}
      <div className="flex items-center">
        {icon}
        <span className="text-gray-700 dark:text-gray-200 ml-3 text-sm">{label}</span> {/* text-sm */}
      </div>
  ...
  return (
    <div className="bg-gray-100 dark:bg-gray-900 min-h-screen flex flex-col">
      ...
      <div className="flex-grow p-3 space-y-2.5 overflow-y-auto pb-4"> {/* Adjusted padding and spacing */}
        ...
        <div className="grid grid-cols-2 gap-2.5"> {/* Reduced gap */}
          <button 
            onClick={handleShareApp}
            className="flex flex-col items-center justify-center bg-white dark:bg-gray-800 p-2.5 rounded-lg shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors" // Reduced padding
            aria-label="Share this app"
            >
            <ShareIcon className="text-indigo-600 dark:text-indigo-400 mb-1 w-5 h-5" /> {/* Adjusted icon size */}
          ...
          <button 
            ...
            >
            <RateUsIcon className="text-indigo-600 dark:text-indigo-400 mb-1 w-5 h-5" /> {/* Adjusted icon size */}
